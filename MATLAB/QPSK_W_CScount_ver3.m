%from old QPSK_W_CScount_ver2.m (without outer for loop over iter)
%counts mean time to nth CS then resets
%23/2 changed to record CStimes for each CS
...
%                     CStime = CStime + 20*iteration;
                    CStimes(CStotal) = ((loopNumber-1)*numBits+20*iteration)/k-sampleSize*2;
%                     disp(CStimes(CStotal))
                    if CStimes(CStotal) < 0%sampleSize*2
                        CStotal = CStotal-1;
                    end
                    loopNumber = 0;
                    CSindicator2 = 1;
